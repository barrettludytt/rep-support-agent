...
- Migrated the knowledge into **~1,940 internal-only Salesforce Knowledge articles** (1,090 cards + 850 Confluence pages), scoped `IsVisibleInPkb=false` so customers/the website can never see them — the internal-vs-public boundary enforced in the data itself.
...
- **Solved a hard grounding problem the "supported" way couldn't:** the standard Lightning knowledge action is hardwired to Data Cloud, and the Data-Library-to-agent binding was broken in the org. Rather than fight it, engineered a **custom Apex SOSL search action** over native Salesforce Knowledge — no Data Cloud dependency, internal-scoped in the query, and fully controllable. Same answer quality, far simpler and more portable.
